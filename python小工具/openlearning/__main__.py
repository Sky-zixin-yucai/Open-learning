#!/usr/bin/env python3
"""
OpenLearning RGA 主模块入口
================================

当使用 `python -m openlearning` 时执行此模块。
...
class UnifiedTestRunner:
    ...
    def _find_project_root(self) -> str:
        ...
        # 向上查找项目根目录（包含openlearning目录）
        for depth in range(5):
            check_dir = os.path.join(current_dir, *['..'] * depth)
            check_dir = os.path.abspath(check_dir)
            
            # 检查是否是openlearning包的根目录
            if os.path.exists(os.path.join(check_dir, 'openlearning')):
                # 检查openlearning是否是一个包（包含__init__.py）
                init_file = os.path.join(check_dir, 'openlearning', '__init__.py')
...
def create_parser():
    ...
    parser = argparse.ArgumentParser(
        prog='python -m openlearning',
        description='OpenLearning RGA - 统一测试运行器',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=textwrap.dedent('''
            示例:
              python -m openlearning demo                 # 运行完整演示
              python -m openlearning test                 # 运行测试模式
              python -m openlearning test-all             # 运行完整测试套件
              python -m openlearning core                 # 运行核心模块测试
              python -m openlearning layers               # 运行层模块测试
              python -m openlearning integration          # 运行集成模块测试
              python -m openlearning check                # 检查模块状态
              python -m openlearning train                # 启动训练菜单
              python -m openlearning infer                # 启动推理测试
              python -m openlearning list                 # 列出所有测试文件
              python -m openlearning run-all              # 运行所有测试
              python -m openlearning interactive          # 交互模式（默认）
            
            子模块调用:
              python -m openlearning core-metrics         # 核心度量计算器
              python -m openlearning core-registry        # 核心注册表
              python -m openlearning layers-attention     # 注意力层
              python -m openlearning layers-balancer      # 平衡器层
              python -m openlearning layers-memory        # 地质记忆层
              python -m openlearning layers-normalization # 归一化层
              python -m openlearning layers-valve         # 单向阀层
              python -m openlearning layers-embeddings    # 嵌入层
              python -m openlearning layers-fusion        # 融合层
